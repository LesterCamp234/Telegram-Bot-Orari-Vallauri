...
def controllo_sabato(doc):
    ...
    # Chiusi -> Settimana A -> 0
    # Aperti -> File con soltanto la settimana B per le prime -> 1
    # Se non c'è nulla, è il file con la settimana B per tutti, comprese le prime -> 2
...
def ottengo_dati_b(doc):
    ...
    for n_pagina in range(len(doc)):
        ...
        intestazione = (page.get_textbox(fitz.Rect(380, 20, 430, 50))
                        .splitlines())
        ...
        for i in range(giorni):
            ...
            if len(blocco1) != 0:
                ...
                if ("(" not in blocco3[len(blocco3) - 1] and "PALESTRA" not in blocco3[len(blocco3) - 1] and blocco3[
                    len(blocco3) - 1] != "Tesauro LAB.INFORMATICA") and "HOME" not in blocco3[len(blocco3) - 1] or ora_singola2 == 1:
...
def ottengo_dati(doc, orario_b):
    ...
    for n_pagina in range(len(doc)):
        ...
        if ("LIC" in classe and orario_b != 0):
            giorni -= 1
        elif orario_b == 1 and ("ELT" not in classe or "MEC" not in classe or "INF" not in classe):
            template_json["sabato"] = {
                "materie": [],
                "professori": [],
                "aule": []
            }
...
def tolgo_unicode(blocco):
    blocco = re.sub(r'(\s+\uea1e)|(\uea1e)|(^s$)', "", blocco).strip().splitlines()
